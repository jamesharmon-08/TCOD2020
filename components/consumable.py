...
class ConfusionConsumable(Consumable):
    ...
    def get_action(self, consumer: Actor) -> Optional[action.Action]:
        self.engine.message_log.add_message("Select a target location.", color.need_target)
    ...
    def activate(self, action: action.ItemAction) -> None:
        ...
        if target is consumer:
            raise Impossible("You can not confuse yourself.")

        self.engine.message_log.add_message(
            f"The eyes of {target.name} look vacant, as it starts to stumble around.", color.status_effect_applied,
        )
...
class FireballDamageConsumable(Consumable):
    ...
    def activate(self, action: action.ItemAction) -> None:
        ...
        if not self.engine.game_map.visible[target_xy]:
            raise Impossible("You must be able to see the target area")

        targets_hit = False
        for actor in self.engine.game_map.actors:
            if actor.distance(*target_xy) <= self.radius:  # in the Area
                self.engine.message_log.add_message(
                ...
                targets_hit = True
        if not targets_hit:
            raise Impossible("There are no targets in the area")
        self.consume()
...
class LightningDamageConsumable(Consumable):
    ...
    def activate(self, action: action.ItemAction) -> None:
        ...
        if target:
            self.engine.message_log.add_message(f"A lightning bolt strikes {target.name}, for {self.damage} damage!"
            )
